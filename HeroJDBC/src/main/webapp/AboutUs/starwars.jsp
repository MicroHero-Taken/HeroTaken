<%@page import="java.util.Date"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"%>
<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
<title>About Us</title>
</head>
<style>
@import url(http://fonts.googleapis.com/css?family=Open+Sans+Condensed:300);
* {
  margin: 0;
  padding: 0;
  box-sizing: border-box;
}

html, body {
  height: 100%;
}

body {
  background: radial-gradient(ellipse at top left, #334455 0%, #112233 100%);
  overflow: hidden;
}

.wrapper {
  position: absolute;
  top: 100%;
  left: 50%;
  width: 400px;
  margin-left: -200px;
  font: 300 30px/1 'Open Sans Condensed', sans-serif;
  text-align: center;
  text-transform: uppercase;
  color: #fff;
  animation: 60s credits linear infinite;
}

.movie {
  margin-bottom: 50px;
  font-size: 50px;
}

.job {
  margin-bottom: 5px;
  font-size: 18px;
}

.name {
  margin-bottom: 50px;
  font-size: 35px;
}

@keyframes credits {
  0% {
    top: 100%;
  }
  100% {
    top: -500%;
  }
}

</style>
<body>
<div class='wrapper'>
  <div class='movie' font-weight="bold">T A K E N</div>
  <div class='job'>組長</div>
  <div class='name'>蘇賢廷</div>
  <div class='job'>組員</div>
  <div class='name'>吳至晟</div>
  <div class='job'>組員</div>
  <div class='name'>郭大偉</div>
  <div class='job'>組員</div>
  <div class='name'>鄭懋霖</div>
  <div class='job'>組員</div>
  <div class='name'>蔡捷安</div>
  <div class='job'>指導老師</div>
  <div class='name'>馬全德</div>
  <div class='job'></div>
  <div class='name' color="#FFC900">--特別感謝--</div>
  <div class='job'>Java</div>
  <div class='name'>王憲春 老師</div>
  <div class='job'>SQL</div>
  <div class='name'>邱華傑 老師</div>
  <div class='job'>HTML</div>
  <div class='name'>紀宜昕 老師</div>
  <div class='job'>JDBC</div>
  <div class='name'>潘麗珍 老師</div>
  <div class='job'>Json,Cloud Computing</div>
  <div class='name'>蘇國鈞 老師</div>
  <div class='job'>EL & Struts 2</div>
  <div class='name'>吳永志 老師</div>
  <div class='job'>JavaScript</div>
  <div class='name'>陳銀華 老師</div>
  <div class='job'>UML,DB</div>
  <div class='name'>劉翰卿 老師</div>
  <div class='job'>CISCO</div>
  <div class='name'> 陳其元  老師</div>
  <div class='job'>director of photography</div>
  <div class='name'>wolfgang petersen</div>
  <div class='job'>story</div>
  <div class='name'>david fincher</div>
  <div class='job'>visual effects supervisor</div>
  <div class='name'>jerry bruckheimer</div>
  <div class='job'>cast supervisor</div>
  <div class='name'>john doe</div>
  <div class='job'>directed by</div>
  <div class='name'>christopher nolan</div>
  <div class='job'>produced by</div>
  <div class='name'>steven spielberg</div>
  <div class='job'>screenplay by</div>
  <div class='name'>michael bay</div>
  <div class='job'>director of photography</div>
  <div class='name'>wolfgang petersen</div>
  <div class='job'>story</div>
  <div class='name'>david fincher</div>
  <div class='job'>visual effects supervisor</div>
  <div class='name'>jerry bruckheimer</div>
  <div class='job'>cast supervisor</div>
  <div class='name'>john doe</div>
  <div class='job'>directed by</div>
  <div class='name'>christopher nolan</div>
  <div class='job'>produced by</div>
  <div class='name'>steven spielberg</div>
  <div class='job'>screenplay by</div>
  <div class='name'>michael bay</div>
  <div class='job'>director of photography</div>
  <div class='name'>wolfgang petersen</div>
  <div class='job'>story</div>
  <div class='name'>david fincher</div>
  <div class='job'>visual effects supervisor</div>
  <div class='name'>jerry bruckheimer</div>
  <div class='job'>cast supervisor</div>
  <div class='name'>john doe</div>
</div>
</body>
</html>